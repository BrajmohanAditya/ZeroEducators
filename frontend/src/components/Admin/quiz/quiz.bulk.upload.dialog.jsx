...
const QuizBulkUploadDialog = ({ isOpen, onClose, quiz }) => {
  ...
  const handleParseRawText = () => {
    ...
    if (result.validCount === 0 && result.errors.length > 0) {
      ...
      toast.success(`Parsed ${result.validCount} questions successfully!`);
    }
  };
  ...
  const handleToggleOptionCorrect = (qIndex, optIndex) => {
    ...
    q.correctAnswerLabel = q.options[optIndex]?.label || "A";
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  Paste questions directly or import via Excel / CSV format.
                </p>
              </div>
            </div>
                ...
                <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200/80 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div>
                    ...
                    <span className="text-[11px] text-slate-500">
                      Questions will be assigned to this section:
                    </span>
                  </div>
                  <select
                    value={defaultSection}
                    onChange={(e) => setDefaultSection(e.target.value)}
                    className="p-2 border border-slate-300 rounded-lg text-xs font-semibold bg-white focus:ring-2 focus:ring-indigo-500 focus:outline-none shrink-0 cursor-pointer"
                  >
...
D. All of the above
Ans: D
Explanation: AI includes Gaming, Expert Systems, and ML/DL.`}
                      </pre>
                    </div>
                  </div>
                  ...
                  <div className="flex justify-end gap-2">
                    <button
                      ...
                      className="flex items-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white rounded-xl text-xs font-bold transition shadow-sm cursor-pointer"
                    >
                ...
                <div className="space-y-4">
                  ...
                  >
                    ...
                    <p className="text-xs text-slate-400 max-w-sm">
                      Supports standard comma-separated (.csv) files exported from Excel or Google Sheets.
                    </p>
                  </div>
                </div>
              )}
                          ...
                          <td className="px-4 py-3">
                            <div className="space-y-1">
                              {q.options.map((opt, oIdx) => (
                                <div
                                  key={oIdx}
                                  onClick={() => handleToggleOptionCorrect(idx, oIdx)}
                                  title="Click to set this option as the correct answer"
                                  className={`flex items-center gap-1.5 px-2 py-0.5 rounded text-[11px] cursor-pointer transition ${
                                    opt.isCorrect
